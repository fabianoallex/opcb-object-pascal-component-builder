...
begin
  ...
  try
    Obj := Builder.Build;
    Assert.IsNotNull(Obj, 'Obj n�o deveria ser nil');
  finally
    Obj.Free;
    Builder.Free;
  end;
...
begin
  ...
  try
    ...
    // Validar n�o-nil
    Assert.IsNotNull(Object_1, 'Object_1 n�o deveria ser nil');
    Assert.IsNotNull(Object_2, 'Object_2 n�o deveria ser nil');
    Assert.IsNotNull(Object_3, 'Object_3 n�o deveria ser nil');
    Assert.IsNotNull(Object_Build, 'Object_Build n�o deveria ser nil');
    ...
    // Validar que todas apontam para a mesma inst�ncia
    Assert.IsTrue(
      (Object_1 = Object_2) and
      (Object_2 = Object_3) and
      (Object_3 = Object_Build),
      'Todas as refer�ncias devem apontar para o mesmo objeto'
    );

  finally
    Object_1.Free; // apenas uma vez, pois todas s�o a mesma inst�ncia
    Builder.Free;
  end;
end;
...
begin
  ...
  Assert.isTrue(Object1 <> Object2, 'Cada Build deve retornar uma nova inst�ncia');
...
begin
  ...
  try
    ...
    Assert.isTrue(
      ...
      'Todas as refer�ncias devem apontar para a mesma inst�ncia'
    );

    // salvar refer�ncia da primeira inst�ncia
    ...
    // validar que � uma nova inst�ncia
    Assert.isTrue(Object_Build <> FirstInstance, 'Cada Build deve retornar uma nova inst�ncia');

    // validar que todas as refer�ncias foram atualizadas
    Assert.isTrue(
      (Object_Build = Object_1) and (Object_1 = Object_2),
      'Todas as refer�ncias devem apontar para a nova inst�ncia'
    );
  finally
    FirstInstance.Free; // inst�ncia da primeira build, j� substitu�da em Object_1/Object_2
    Object_Build.Free;  // inst�ncia da segunda build (= Object_1 = Object_2)
    Builder.Free;
  end;
end;
...
begin
  ...
  try
    ...
    // Resetar refer�ncias e usar novas
    Builder.ResetReferences;
    Builder.Assign(Object_3).Build;

    // validar que a primeira build n�o foi sobrescrita
    Assert.IsNotNull(Object_1, 'Object_1 n�o deveria ser nil');
    Assert.IsNotNull(Object_2, 'Object_2 n�o deveria ser nil');

    // validar que a segunda build foi atribu�da apenas � nova refer�ncia
    Assert.IsNotNull(Object_3, 'Object_3 n�o deveria ser nil');
    ...
    Object_1.Free; // = Object_2 (mesma inst�ncia da primeira build)
    Object_3.Free;
    Builder.Free;
  end;
end;

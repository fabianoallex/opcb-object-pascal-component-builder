...
begin
  ...
  try
    ControlBuilder
      ...
      .AddControl(TControlInfo.Create(TLabel).WithCaption('Nome de usuário'))
      .AddControl(TControlInfo.Create(TEdit, 'EditUserName', FEditUserName).WithText('').WithWidth(250))
      .AddControl(TControlInfo.Create(TLabel).WithCaption('Senha'))
      .AddControl(TControlInfo.Create(TEdit, 'EditPassword', FEditPassword).WithText('').WithWidth(250).Setup(@SetupEditPassword))
      .SetDirection(cpdHorizontal)
      .IncTop(20)
      .AddControl(TControlInfo.Create(TBitBtn, 'ButtonOk').WithWidth(75).Setup(@SetupButtons))
      .AddControl(TControlInfo.Create(TBitBtn, 'ButtonCancel').WithWidth(75).Setup(@SetupButtons))

...
var
  ...
  I: Integer;
  Filler: IGridFill; // TGridLayoutFillerFactory;
begin
  ...
  Filler := TGridLayoutFillerFactory.Create
    .CreateFiller(ftRowFirst, FGridLayout);

  for I:=0 to FControls.Count-1 do
  begin
    ControItem := TControlItem(FControls.Items[I]);
    if not Assigned(ControItem) then
      Continue;

    Filler.PlaceItem(
      TControlGridItem.Create(ControItem.Control),
      TGridCellSettings.Create
